...
static int ganye_clock_test(struct sgtl5000_priv *sgtl5000, struct i2c_client *client)
{
	unsigned long cur_clk_hz = 0;
	
	sgtl5000->mclk = devm_clk_get(&client->dev, NULL);
	if (IS_ERR(sgtl5000->mclk)) {
		GANYE_PRINTK("get clk error!!");
		return -1;		
	}
	cur_clk_hz = clk_get_rate(sgtl5000->mclk);
	GANYE_PRINTK("cur_clk_hz= %ld", cur_clk_hz);
	return 0;
}
			      ...
			      const struct i2c_device_id *id)
{
	...
	#if 0	//add by chenqigan
			ganye_clock_test(sgtl5000, client);
	#endif
