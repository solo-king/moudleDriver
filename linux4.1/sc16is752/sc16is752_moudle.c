...
#define CONFIG_SERIAL_SC16IS7XX_SPI //add by chenqigan
//#define CONFIG_SERIAL_SC16IS7XX_I2C //add by chenqigan
#define SC16IS752_CLK		18432000

#define UART_DEV_NAME	"COM"
				...
				unsigned int iir)
{
	...
	while (rxlen) {
		...
		if (read_lsr) {
			...
			regcache_cache_bypass(s->regmap, true);
			regmap_raw_read(s->regmap, SC16IS7XX_RHR_REG|port->line,
					s->buf, rxlen);
			regcache_cache_bypass(s->regmap, false);
			bytes_read = rxlen;
		}
...
static void sc16is7xx_handle_tx(struct uart_port *port)
{
	...
	if (likely(to_send)) {
		...
		regcache_cache_bypass(s->regmap, true);
		regmap_raw_write(s->regmap, SC16IS7XX_THR_REG|port->line, s->buf, to_send);
		regcache_cache_bypass(s->regmap, false);
	}
...
static int sc16is7xx_spi_probe(struct spi_device *spi)
{
	...
	GANYE_PRINTK("before sc16is7xx_probe..");
